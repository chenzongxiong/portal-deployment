function transforming(input) {
  ...
  output.title = [
    {
      "@value": input["oai_dc:dc"]["dc:title"],
      "@language": params.defaultLanguage,
    },
  ];

  if(input["oai_dc:dc"]["dc:description"]){
    output.description = [
      {
        "@value": input["oai_dc:dc"]["dc:description"],
        "@language": params.defaultLanguage,
      },
    ];}

  if(input["oai_dc:dc"]["dc:type"]){
    output.type = [
      {
        "@value": input["oai_dc:dc"]["dc:type"],
        "@language": params.defaultLanguage,
      },
    ];}

  if (input["oai_dc:dc"]["dc:identifier"]){
      output.identifier = [
      {
        "@value": input["oai_dc:dc"]["dc:identifier"],
        "@language": params.defaultLanguage,
      },];
    }

  if (input["oai_dc:dc"]["dc:creator"]){
    output.contactPoint = [
      {
        "@type": "Kind",
        "http://www.w3.org/2006/vcard/ns#fn": input["oai_dc:dc"]["dc:creator"],
      },
    ];}
  if (input["oai_dc:dc"]["dc:creator"]) {
    output.creator = [
      {
        name: input["oai_dc:dc"]["dc:creator"],
      },
    ];}

  if (input["oai_dc:dc"]["dc:format"]) {
    output.format = [
      {
        name:input["oai_dc:dc"]["dc:format"],
      },
    ];}


  if (input["oai_dc:dc"]["dc:rights"]) {
    rights = [];
    if (input["oai_dc:dc"]["dc:rights"]) {
      if (typeof input["oai_dc:dc"]["dc:rights"] === "object") {
        for (const right of input["oai_dc:dc"]["dc:rights"]) {
          rights.push({
            "@value": right,
            "@language": params.defaultLanguage,
          });
      ...
      rights = input["oai_dc:dc"]["dc:rights"];

    }
  }
}
if (input["oai_dc:dc"]["dc:relation"]) {
  output.distribution = [];
  if (input["oai_dc:dc"]["dc:relation"]) {
    if (typeof input["oai_dc:dc"]["dc:relation"] === "object") {
      for (const relation of input["oai_dc:dc"]["dc:relation"]) {

        output.distribution.push({
          "@type": "Distribution",
          license: rights,
          accessURL: {
            "@id": encodeURI(decodeURI(relation)),
          },
          title:[
            {
              "@value": relation,
      ...
      output.distribution = [
        {
          "@type": "Distribution",
          license: rights,
          identifier:input["oai_dc:dc"]["id"],
          format: input["oai_dc:dc"]["format"],
          accessURL: {
            "@id": encodeURI(decodeURI(input["oai_dc:dc"]["dc:relation"])),
          },
          title:[
            {
              "@value": input["oai_dc:dc"]["dc:title"],
  ...
  if (input["oai_dc:dc"]["dc:subject"]) {
    output.keyword = [];
    if (input["oai_dc:dc"]["dc:subject"]) {
      if (typeof input["oai_dc:dc"]["dc:subject"] === "object") {
        for (const subject of input["oai_dc:dc"]["dc:subject"]) {
          output.keyword.push({
            "@value": subject,
            "@language": params.defaultLanguage,
          });
      ...
      output.keyword = [
        {
          "@value": input["oai_dc:dc"]["dc:subject"],
          "@language": params.defaultLanguage,
        },
      ];
    }}
  ...
  if (input["oai_dc:dc"]["dc:publisher"]) {
    output.publisher = {
      "@type": "Agent",
      name: input["oai_dc:dc"]["dc:publisher"],
      type: {
